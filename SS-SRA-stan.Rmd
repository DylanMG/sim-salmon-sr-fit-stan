...
##dylan code questions/comments are in chunks preceeded by '##'
```
...
```{r}
##these returns by age 1-4 are CALENDAR YEAR? but N is referenced by brood year?
head(sim_data)
```
...
```{r}
## ASSUMING this is based on calendar year from a sampling perspective. 
...
```{r}
S_obs <- rnorm(length(sim_data$S), ## is this just adding some noise to the simulated data? not sure that the SD is being treated right here, shouldn't it be sim_data_S*0.2 to convert from cv to sd?
               sim_data$S,
               0.2)
...
nRyrs <- nyrs + A - 1 # number of recruitment years ## WHYYYY? are the xtra years at start or end. End?
...
spwn.quant <- apply(spwn, 2, quantile, probs=c(0.05,0.5,0.95))[,1:(sr_years-4)] ##MAKE ROBUST AND UNDERSTAND WHY
