...
##demonstrate rowwise is redundant - it's built into dplyr syntax already 
A_obs2 <- as.data.frame(sim_data) %>%
  mutate(age.4=N.age.1/N,
         age.5=N.age.2/N,
         age.6=N.age.3/N,
         age.7=N.age.4/N) %>%
...
A_obs2 <- round(as.matrix(A_obs2)*100)
rm(A_obs2) ##remove the example
```

Next we need to generate data frames of spawner and harvest observations, in this case by assuming a CV (DG notes this is a SD not a CV!) of 20% for spawners and 5% for harvest:
