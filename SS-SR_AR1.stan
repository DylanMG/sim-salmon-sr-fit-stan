...
data{
  ...
  int n_R_y;           // number of recruitment years
...
parameters{
  vector<lower=0>[n_R_y] lnR;             // log recruitment states
  ...
  matrix<lower=0.01>[n_R_y, n_a] g;       // individual year/age class gamma variates for generating age at maturity proportions
}
...
transformed parameters{
  ...
  vector<lower=0>[n_R_y] R;             // recruitment states
  vector[n_R_y] lnresid;                // log recruitment residuals
  vector[n_R_y] lnRm_1;                 // log recruitment states in absence of lag-one correlation
  vector[n_R_y] lnRm_2;                 // log recruitment states after accounting for lag-one correlation
  matrix<lower=0>[n_y, n_a] N_ta;        // returns by age matrix
  matrix<lower=0, upper=1>[n_R_y, n_a] p; // age at maturity proportions
...
model{
  ...
  // State model
  lnR[(n_a + a_min):n_R_y] ~ normal(lnRm_2[(n_a + a_min):n_R_y], sigma_R);
